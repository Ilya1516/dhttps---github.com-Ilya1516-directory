...
choice_main_menu_error = f'Нужно ввести число от1 до {1}-{len(main_menu)-1}!'
...
input_id_for_edet = 'Введите ID контактаб который хотите изменить'
input_id_for_delete = 'Введите ID контактаб который хотите удалить'

def new_contact_added_successful(name: str) -> str:
    return f'Контакт "{name}" успешно добаилен'

def find_contact_no_result(word: str) -> str:
    return f'Контакты содержащие "{word}" не найдены!'


def edit_contact_successfui(name) -> str:
    return f'Контакт "{name}" успешно изменен!'

def delete_contact_successfui(name) -> str:
